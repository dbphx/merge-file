...
func ExtractFolderID(rawURL string) (string, error) {
	...
	if plainFolderIDPattern.MatchString(candidate) {
		return candidate, nil
	}

	parsed, err := url.Parse(candidate)
	if err == nil {
		if id := strings.TrimSpace(parsed.Query().Get("id")); plainFolderIDPattern.MatchString(id) {
			return id, nil
		}

		segments := strings.Split(strings.Trim(parsed.Path, "/"), "/")
		for index := 0; index < len(segments)-1; index++ {
			if segments[index] != "folders" {
				continue
			}
			id := strings.TrimSpace(segments[index+1])
			if plainFolderIDPattern.MatchString(id) {
				return id, nil
			}
		}
	}
...
func (c Client) PreviewFolder(ctx context.Context, folderURL string) ([]model.DrivePreviewFile, error) {
	...
	for _, file := range payload.Files {
		if _, ok := supportedDriveMimeTypes[file.MimeType]; !ok {
			continue
		}
...
// DownloadFile streams a shared Drive file into the merge workspace without persisting source files.
